...
public class LoopingTest {
    ...
    @Test
    public void jdk8For() {
        employees.stream().filter(emp -> emp.getAge() < age).forEach(emp -> System.out.println(emp));
    }

    @Test
    public void mapVariations() {
        alpha.stream().map(String::toUpperCase).forEach(val -> System.out.println(val));
        List<String> collect = alpha.stream().map(String::toUpperCase).collect(Collectors.toList());
        System.out.println(collect);
        employees.stream().map(x -> x.getName()).collect(Collectors.toList());
        System.out.println(collect);
